...
class SnowflakeFileMeta:
    ...
    client: Optional[
        Union["Session.resource", "BlobServiceClient", Optional[str]]
    ] = None
...
class SnowflakeFileTransferAgent(object):
    ...
    def _transfer_accelerate_config(self):
        if self._stage_location_type == S3_FS:
            ...
            logger.debug("use_accelerate_endpoint: %s", self._use_accelerate_endpoint)
        ...
        while idx < len_file_metas:
            ...
            while True:
                ...
                results: List["SnowflakeFileMeta"] = list(
                    pool.map(SnowflakeFileTransferAgent.upload_one_file, target_meta)
                )
                pool.shutdown()
